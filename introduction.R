...
To get a clear picture of what is happening in the dataset, it might be a valueble thing to do a 
cluster analysis. This will produce a nice picture of groups of genes that react(are regulated) in a simular way.<br>
In this application a number of different methods of clustering is combined to have a quick look wich fits yoou data best.
<br>
<br>
<h3>Input</h3><br>
...
only the read counts (tab delimitted).</blockquote></li>
<li> A sample file (or design file) that should be a tab delimited file containing two or three columns with header.
First comlumn named sample containing the sample names <b><u>identical</u></b> in <u>name</u> and <u>order</u> as the sample names in the 
...
different methods with different settings and see what looks best. The clustering is visualized by a headmap with a color bar indicating the cluster gens are clustered to,
